    ...
    if 'Fields' in headers.keys():
        ...
        for field in fields:
            if field not in config['tables_fields'][table]:
                return {"status": "error", "code": 404, "message": f"Field <{field}> not found.", "token": create_token(jwt.decode(headers['Token'], current_app.config['SECRET_KEY'], algorithms='HS256')['user'])}
    else:
    ...
    for mandatory_header in config['mandatory_headers'][route_name]:
        if mandatory_header not in headers.keys():
            if token is not None:
                return {"status": "error", "code": 402, "message": f"Required field <{mandatory_header}> is not specified.", "token": create_token(jwt.decode(headers['Token'], current_app.config['SECRET_KEY'], algorithms='HS256')['user'], token)}
            else:
                return {"status": "error", "code": 402, "message": f"Required field <{mandatory_header}> is not specified."}
    for key in headers.keys():
        if len(headers[key]) == 0:
            if token is not None:
                return {"status": "error", "code": 402, "message": f"Required field <{key}> is empty.", "token": create_token(jwt.decode(headers['Token'], current_app.config['SECRET_KEY'], algorithms='HS256')['user'], token)}
            else:
                return {"status": "error", "code": 402, "message": f"Required field <{key}> is empty."}
    if token is not None:
        return {"status": "success", "code": 200, "message": f"All the required field are specified.", "token": create_token(jwt.decode(headers['Token'], current_app.config['SECRET_KEY'], algorithms='HS256')['user'], token)}
    return {"status": "success", "code": 200, "message": f"All the required field are specified."}
...
def create_token(user: str, old_token=None):
    logger.info('create_token')
    logger.debug('user: {}', user)
    logger.debug('token: {}', old_token)
    token = jwt.encode({'user': user, 'exp': datetime.datetime.utcnow() + datetime.timedelta(hours=config['token']['duration'])}, current_app.config['SECRET_KEY'])
    if token is not None:
        token_to_blacklist(old_token)
    return token
